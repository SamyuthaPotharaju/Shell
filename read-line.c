...
int history_length = HISTORY_SIZE; //sizeof(history)/sizeof(char *);
...
void read_line_print_usage()
{
  char * usage = "\n"
    ...
    " ctrl-h    remove character at position before cursor\n"
  ...
  while (1) {
    ...
    if (ch>=32 && ch != 127) {
      ...
      if (line_length != 0) {
          ...
          new_history_index = history_index;
          history_index++;
	...
	// Print spaces on top
	for (i = 0; i < line_length + right_length; i++) {
	  ch = ' ';
	  write(1,&ch,1);
	}
    ...
    right_length = 0;
	    ...
	    strcpy(line_buffer, history[new_history_index]);
	    line_length = strlen(line_buffer);
        int temp = history_full?history_length:history_index;
        int pos = ch2 == 65? -1:1;
        new_history_index = (new_history_index+pos) % temp;

        if (new_history_index == -1) {
            new_history_index = temp-1;
        }
